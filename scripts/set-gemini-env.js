...
async function main() {
  ...
  const apiUrlArg = process.argv[4] || process.env.NEW_GEMINI_API_URL || "";
  const apiKey = String(apiKeyArg || await ask("Paste Gemini API key and press Enter: ")).trim();
  const model = String(modelArg || await ask("Gemini model [gemini-flash-lite-latest]: ")).trim() || "gemini-flash-lite-latest";
  const apiUrl = String(apiUrlArg || "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions").trim();
  ...
  console.log(`Saved GEMINI_API_URL=${apiUrl}`);
