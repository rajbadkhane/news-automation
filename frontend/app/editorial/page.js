...
async function getEditorialFeed() {
  try {
    const response = await fetch(`${API_BASE_URL}/api/v1/editorial/grouped?limit=15`, {
      cache: "no-store",
      headers: {
...
export default async function EditorialPage() {
  const initialPayload = await getEditorialFeed();

  return <NewsTableDesk initialPayload={initialPayload} initialSection="editorial" />;
}
